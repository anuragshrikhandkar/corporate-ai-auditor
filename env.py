"""
Corporate AI Auditor — env.py  GUARANTEED FIX
All scores strictly in (0.01, 0.99) — no exceptions
"""
import uuid
...
def _c(v: float) -> float:
    """Clamp any float strictly into (0.01, 0.99)."""
    try:
        v = float(v)
    except Exception:
        return _LO
...
SYSTEM_TASK3 = AISystem(
    ...
    data_sources=[
        "Credit bureau data", "Bank transaction history", "Social network data",
    ...
    flags=[
        "ecoa_violation_risk", "disparate_impact_detected", "no_explainability",
...
DOCS_TASK1 = {
    "model_card": (
        "Model: XGBoost classifier. Features: 47 input features including university name, "
    ...
    "feature_importance": (
        "Top features: 1) University prestige score (0.31) 2) Previous company tier (0.24) "
        "3) Graduation year (0.18) 4) Zip code cluster (0.12) 5) Name-gender inference score (0.09)."
    ),
    "training_data_report": (
        "Training labels from historical hiring decisions 2015-2022. "
        "Male hire rate: 73%. Female hire rate: 52%. Minority hire rate: 38%. No re-balancing applied."
    ),
    "vendor_contract": (
        "SLA: 99.9% uptime. Audit rights: limited to aggregate statistics. "
...
DOCS_TASK2 = {
    "data_inventory": (
        "Personal data: Name, DOB, SSN, diagnosis codes, medications, lab results. "
        "Social media handles collected in 67% of cases. Storage: AWS S3 unencrypted. Retention: indefinite."
    ),
    "consent_forms": (
        "Patients sign general treatment consent only. No specific AI processing consent. "
        "No mention of data sharing with MedML Solutions. No opt-out mechanism. Last updated: 2019."
    ),
    "data_sharing_log": (
        "Data shared with: MedML Solutions, 3 research universities, 2 pharmaceutical companies. "
        "De-identification: remove name+SSN only. Re-identification risk: high."
    ),
    "security_report": (
        "Last pentest: 2021. Critical issues: unencrypted S3, no audit logging, "
...
DOCS_TASK3 = {
    ...
    "disparate_impact_analysis": (
        "Approval rates: White 68%, Hispanic 41%, Black 38%, Asian 61%. "
        "Disparate impact ratio: 0.56 — below 0.80 legal threshold. "
        "Geographic denial correlates with minority zip codes."
    ),
    "data_sources_detail": (
...
TASK_REGISTRY: Dict[str, Dict] = {
    "bias_detection": {
        ...
        "description": (
            "Audit TalentScreen Pro for bias and fairness violations. "
    ...
    "privacy_compliance": {
        ...
        "description": (
            "Audit PatientPredict for GDPR/HIPAA privacy compliance. "
    ...
    "full_risk_audit": {
        ...
        "description": (
            "Full regulatory audit of CreditOracle v2. "
...
def _best(actions, spec) -> float:
    raw = max((_match(a, spec) for a in actions), default=0.0)
    return _c(raw)


def grade_task1(actions: List) -> Tuple[float, Dict, str]:
    s = {k: _best(actions, spec) for k, spec in TASK1_FLAGS.items()}
    total = _c(sum(s.values()) / len(TASK1_FLAGS))
    return total, s, f"{sum(1 for v in s.values() if v > 0.5)}/{len(TASK1_FLAGS)} found"


def grade_task2(actions: List) -> Tuple[float, Dict, str]:
    s = {k: _best(actions, spec) for k, spec in TASK2_FLAGS.items()}
    total = _c(sum(s.values()) / len(TASK2_FLAGS))
    return total, s, f"{sum(1 for v in s.values() if v > 0.5)}/{len(TASK2_FLAGS)} found"


def grade_task3(actions: List) -> Tuple[float, Dict, str]:
    s = {k: _best(actions, spec) for k, spec in TASK3_FLAGS.items()}
    total = _c(sum(s.values()) / len(TASK3_FLAGS))
    return total, s, f"{sum(1 for v in s.values() if v > 0.5)}/{len(TASK3_FLAGS)} found"
...
class AIAuditorEnv:
    ...
    def _apply_action(self, action: Action):
        if action.action_type == "request_document":
            ...
            "flag_bias", "flag_privacy", "flag_security",
