"""
Corporate AI Auditor — env.py (FINAL FIX)
All scores strictly between 0.01 and 0.99 — never 0.0 or 1.0
"""
...
DOCS_TASK1 = {
    "model_card": ("Model: XGBoost classifier. Features: 47 input features including "
    ...
    "vendor_contract": ("SLA: 99.9% uptime. Model updates: quarterly. "
        "Audit rights: limited to aggregate statistics. Data retention: 5 years. "
        "Right to explanation: not included."),
}
DOCS_TASK2 = {
...
TASK_REGISTRY: Dict[str, Dict] = {
    ...
    "full_risk_audit": {
        ...
        "description": ("Conduct a full regulatory audit of CreditOracle v2. "
            "Identify ECOA/CFPB violations, disparate impact, privacy breaches, explainability failures. "
            "You must: request documents, flag all violations, assess risk, AND write a recommendation."),
...
def _match_finding(action, spec) -> float:
    """Returns strictly > 0 always — KEY FIX."""
    if action.action_type != spec["action"]:
        return _LO  # never 0.0
...
def _best(actions, spec) -> float:
    """Best match score, always strictly in (0,1)."""
    if not actions:
        return _LO
    return _clamp(max(_match_finding(a, spec) for a in actions))


def grade_task1(actions) -> Tuple[float, Dict, str]:
    ...
    return total, scores, f"{found}/{len(TASK1_REQUIRED_FLAGS)} bias issues identified."


def grade_task2(actions) -> Tuple[float, Dict, str]:
    ...
    found = sum(1 for v in scores.values() if v >= 0.5)
    return total, scores, f"{found}/{len(TASK2_REQUIRED_FLAGS)} privacy issues identified."


def grade_task3(actions) -> Tuple[float, Dict, str]:
    scores = {}
    ...
    scores["risk_assessment"] = _best(risk_actions, TASK3_REQUIRED_FLAGS["risk_assessment"]) if risk_actions else _LO
    rep_actions = [a for a in actions if a.action_type == "write_recommendation"]
    if rep_actions:
        best_r = max(_match_finding(a, TASK3_REQUIRED_FLAGS["written_report"]) for a in rep_actions)
        lb = min(0.20, len(max(rep_actions, key=lambda a: len(a.value)).value) / 500)
        scores["written_report"] = _clamp(best_r + lb)
    else:
        scores["written_report"] = _LO
    total = _clamp(sum(scores.values()) / len(TASK3_REQUIRED_FLAGS))
    found = sum(1 for v in scores.values() if v >= 0.5)
    return total, scores, f"{found}/{len(TASK3_REQUIRED_FLAGS)} audit components completed."
...
class AIAuditorEnv:
    ...
    def _compute_step_reward(self, action: Action) -> float:
        ...
        return _clamp(delta + doc_bonus - penalty + _LO)
