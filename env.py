"""
Corporate AI Auditor — env.py (NO DIVISION, GUARANTEED RANGE)
All scores and rewards strictly in [0.01, 0.99]
"""
...
def clamp(x: float) -> float:
    """Return exactly MIN_SCORE or MAX_SCORE if out of bounds, else rounded to 4 decimals."""
    if x <= 0.0:
        return MIN_SCORE
    if x >= 1.0:
        return MAX_SCORE
...
# ============ SYSTEM DEFINITIONS (same as yours) ============
...
def _best(actions, spec):
    if not actions:
        return MIN_SCORE
    return clamp(max(_match_finding(a, spec) for a in actions))
...
def grade_task1(actions):
    ...
    return clamp(total), scores, f"{sum(1 for v in scores.values() if v>0.5)}/{len(TASK1_REQUIRED_FLAGS)} issues"
...
def grade_task2(actions):
    ...
    return clamp(total), scores, f"{sum(1 for v in scores.values() if v>0.5)}/{len(TASK2_REQUIRED_FLAGS)} issues"
...
def grade_task3(actions):
    ...
    return clamp(total), scores, f"{sum(1 for v in scores.values() if v>0.5)}/{len(TASK3_REQUIRED_FLAGS)} issues"
